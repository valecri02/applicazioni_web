...
def new_comment():
  ...
  if picture.filename != '':
    picture.save('static/' + picture.filename)
  addcomment['pic'] = picture.filename

  n_post = addcomment['id_post']
  del addcomment['id_post']

  for post in posts:
    if post['id'] == int(n_post) and valid_comment(addcomment['user'], addcomment['text'], addcomment['evaluation']):
      post['comments'].append(addcomment)
      app.logger.info(post)

  return redirect(url_for('post_page', id=n_post))